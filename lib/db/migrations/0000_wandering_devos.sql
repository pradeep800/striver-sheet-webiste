...
CREATE TABLE `striver_sheet_reminders` (
	...
	`should_send_mail` boolean NOT NULL,
	`mail_sended` boolean NOT NULL DEFAULT false,
	`user_id` varchar(255) NOT NULL,
	`question_no` int NOT NULL
);
--> statement-breakpoint
...
CREATE INDEX `created_at_index` ON `striver_sheet_ai_chat_messages` (`created_at`);--> statement-breakpoint
...
CREATE INDEX `user_id_idx` ON `striver_sheet_tracking_questions` (`user_id`);